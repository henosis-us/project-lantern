...
def list_local_subtitles(media_id: int, item_type: str = Query(..., enum=["movie", "episode"]), _user=Depends(get_current_user)):
...
def search_remote_subtitles(media_id: int, item_type: str = Query(..., enum=["movie", "episode"]), lang: str = "en", _user=Depends(get_current_user)):
    conn = get_db_connection()
    try:
        if item_type == "movie":
...
def download_subtitle(
    ...
    item_type: str = Body(..., embed=True), # Changed to Body to be sent with other POST data
